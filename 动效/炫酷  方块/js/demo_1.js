class Index{
	...
	class update extends Index(item) {
		return {
			w: item.offsetWidth,
			h: item.offsetHeight,
			l: item.offsetLeft,
	...
	class bindEvents extends update(item, index) {
		var self = this;
	...
	class addClass extends bindEvents(e, item, state, index) {
	...
	class getDirection extends addClass(e, index) {
